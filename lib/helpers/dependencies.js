...
function findAllReferences(node, scope) {
  ...
  _.map(node, function (value, key) {
    if (_.isString(value)) {
      ...
      var subNodeVariableIndex = findAllReferences(value);
      var blocking = [];

      for (var subNodeKey in subNodeVariableIndex) {
        var thisKeysVariables = subNodeVariableIndex[key];
        for (var subNodeVariable in thisKeysVariables) {
          if (subNodeVariable.parent) {
            blocking.push({
              dependency: subNodeVariable.dependency
            });
          }
        }
      }
